...
description: Structured debugging protocol for hard bugs. Builds a tight feedback loop first, then reproduces, hypothesises, instruments, fixes, and cleans up. Use when a bug is non-obvious, flaky, or has resisted initial investigation.
---
...
A correct seam is one where the test exercises the **real bug pattern** as it occurs at the call site. If no correct seam exists, that itself is the finding — note it and flag for architecture review.
...
**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling), flag it for architecture review with the specifics. Make the recommendation **after** the fix is in, not before.
